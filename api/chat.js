...
const AUTHORIZED_EMAIL = "karanramnani201@gmail.com";
...
// ─── Keyword search ──────────────────────────────────────────────────────────
// Returns top N remedies whose indication text contains the most query keywords.
function searchRemedies(keywords, topN = 20) {
  ...
  const scored = remedies.map(r => {
    ...
    let score = 0;
    for (const kw of kws) {
      // count occurrences — more mentions = stronger match
      let idx = 0;
...
function formatRemediesForPrompt(remedies) {
  if (!remedies.length) return "No remedies matched for these keywords.";
  return remedies
    .map((r, i) =>
...
// The system prompt instructs the model to output "READY_FOR_REMEDIES: kw1, kw2, ..."
...
module.exports = async function handler(req, res) {
  ...
  // Server-side subscription gate. The browser UI is not trusted for access control.
  let user;
  try {
  ...
  if (!user) {
    return res.status(401).json({ error: "Please sign in to use HomeoHelp." });
  }
  ...
  const { messages = [], model = "openai/gpt-oss-20b" } = body;
  const selectedModel = CHAT_MODELS.has(model) ? model : "openai/gpt-oss-20b";
  ...
  try {
    ...
    if (!readyKeywords) {
      // Still gathering symptoms — return the question to the user
      return res.status(200).json({ reply: phase1Reply, phase: "questioning" });
    }

    // ── PHASE 2: search CSV + rank remedies ──
